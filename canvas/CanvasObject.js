...
export class CanvasObject extends EventEmitter {
  ...
  constructor(context = new SVGCanvas(), type = '', options) {
    super();
    const { id, model = {}, effects } = options;
  ...
  #valuesMatch(left, right) {
    if (left?.equals && typeof left.equals === 'function' && CanvasPoint.isPointLike(right)) {
      return left.equals(right);
    }
    
    if (right?.equals && typeof right.equals === 'function' && CanvasPoint.isPointLike(left)) {
