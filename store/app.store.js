...
const toolbarEnabledState = ref(false);
...
export const useAppState = () => {
  ...
  const setToolbarEnabled = (value) => {
    if (typeof value !== 'boolean') {
      console.error(`setToolbarEnabled called invalid value: ${value}`)

      return;
    }

    toolbarEnabledState.value = value;
  };
  ...
  const toggleToolbar = () => {
    setToolbarEnabled(!toolbarEnabledState.value);
  };
  ...
  return {
    ...
    toolbarEnabled: computed(() => toolbarEnabledState.value),
