...
export class GraphNode extends EventEmitter {
	...
	constructor(data = {}) {
		super();
		const { tileType, x, y, target, linkedMap, active, current, address } = data
		...
		this.#data.id = `${this.x}_${this.y}`;
		this.#data.address = this.id;
	}
	...
	update(attributeMap = {}) {
		const patch = Object.entries(attributeMap).reduce((ptch, [k, v]) => {
			...
			if (isValid && v !== modelV) {
				...
				console.error(`[GraphNode] invalid Model patch: ${k}: ${v}`)
			}
