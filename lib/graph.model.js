...
export class GraphNode extends EventEmitter {
	...
	#linkedNodeAddress = null;
	#linkedMap = null;
	#data = new Map(DETAULT_TILE_DATA)
	
	constructor({ tileType, x, y, target, linkedMap, active, current }) {
		super();
	...
	get tileType() { return this.#data.get('tileType'); }
	
	set tileType(v) { this.#data.set('tileType', v); }
	...
	update(attributeMap = {}) {
		Object.entries(attributeMap).forEach(([k, v]) => {
			if (this.#data.has(k)) {
				this.#data.set(k, v);
			}
		});
		
		this.emit('node:update', { id: this.id, data: this.data() })
		return this;
	}
	...
	toJSON() {
		const res2 = {
			...
			id: this.id,
			tileType: this.#data.get('tileType'),
			current: this.#data.get('current'),
			active: this.#data.get('active'),
			address: this.#data.get('address'),
			x: this.x, //this.#data.get('x'),
			y: this.y, //this.#data.get('y'),
			isTraversable: this.#data.get('isTraversable'),
			isPathNode: this.#data.get('isPathNode'),
			isVisited: this.#data.get('isVisited'),
			previous: this.#data.get('previous'),
			linkedNodeAddress: this.#data.get('linkedNodeAddress'),
			target: this.#data.get('target'),
			linkedMap: this.#data.get('linkedMap'),
		};
...
export class Graph extends EventEmitter {
	...
	get startNode() { return this.#startNode ?? this.nodes.find(n => this.previousMapId ? n.linkedMap === this.previousMapId : n.tileType === 'start'); }
	...
	pathToDirections(path = []) {
		...
		for (let i = 1; i < path.length; i++) {
			...
			// try spatial first (your helper)
			let dir = getDirectionFromPoints(prev, curr)
			
			// fallback for non-spatial edges (teleport, etc.)
			if (!dir) {
	...
	* traverseHybrid(start = this.startNode, getGoal = () => this.goalNode) {
		...
		while (true) {
			...
			// fallback if world changed too much
			if (!next) {
				path = this.getPath(current, goal) || []
				dirs = this.pathToDirections(path)
				i = 0
